...
def sentencesplitlang(file_content):
    ...
    file_content = re.sub(r"(w+)\b/(w+)\b", r"\1\2",file_content)
    ##
    doc = sanitize(re.sub(r'(?m)^\@.*\n?',' ',file_content.replace('’', '\'').replace('‘', '\'').replace('…',' ... ').replace('«',' " ').replace('»',' " ').replace('<BLOCK>',' ')))
    sentences_with_foreslash = sent_tokenize(doc,language=punkt)
    for s in sentences_with_foreslash:
        if "/" in s:
            sentences_with_foreslash.remove(s)
    sentences=enumerate(sentences_with_foreslash,1)
    return sentences
