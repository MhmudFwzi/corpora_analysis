...
def sentencesplitlang(file_content):
    file_content = re.sub(r'<page.*?>|</page.*?>|<normalised.*?>|</normalised.*?|<inferred.*?>|</inferred.*?>', "", file_content)
    root = ET.fromstring(file_content)
    sentences = []
    word_expression = re.compile("[a-zA-Z0-9]+.*")
    for child in root:
        sentence = ""
        for i,r in enumerate(child.text.split("\n")):
            try:
                next_token = r.split("\t")[0]
                if word_expression.match(next_token):
                    if i != 1:
                        sentence += " " + next_token
                    else: sentence += next_token
                else:
                    sentence += next_token
            except:
                pass
        sentences.append(sentence)
    return enumerate(sentences,1)
            ...
            if syntacticrel in left:
                for ident,token in left[syntacticrel].items():
                    for x in sentence:
                        if token.head == x.id:
                            head = x
                    countwriter.writerow(['X-'+syntacticrel,head.deprel,token.deprel,head.form,token.form,head.lemma,token.lemma,head.upos,token.upos,sentence.id,conllufile.split(seppath)[3].split('.')[0],conllu.__len__()])
            if syntacticrel in right:
                for ident,token in right[syntacticrel].items():
                    for x in sentence:
                        if token.head == x.id:
                            head = x
                    countwriter.writerow([syntacticrel+'-X',token.deprel,head.deprel,token.form,head.form,token.lemma,head.lemma,token.upos,head.upos,sentence.id,conllufile.split(seppath)[3].split('.')[0],conllu.__len__()])
    return
...
def main():
    ...
    if args.parse:
        ...
        nlp = loadmodel(ud)
        #for filename in tqdm(sorted(glob.glob(args.source+'/*.txt'))):
        for filename in tqdm(sorted(glob.glob(args.source+'/*.vrt'))):
    ...
    if args.analyze:
        ...
        countwriter.writerow(['order', 'relfirst', 'relsecond', 'tokenfirst', 'tokensecond', 'lemmafirst', 'lemmasecond', 'uposfirst', 'upossecond', 'sentence', 'source', 'size'])
